...
MNV ima asimptotsko najmanjšo varianco (je učinkovita), kar sledi kot posledica neenakosti Cramer-Raa. V primeru diskretne porazdelitve je verjetje kar produkt verjetnosti: $$L(x;\ \theta) = \prod_{i=1}^n P(X_{i} = x_{i};\ \theta).$$

Za izpeljavo cenilke vektorja koeficientov si poglejmo primer osnovnega modela linearne regresije v matrični obliki $Y=X\beta + \epsilon$, kjer so
...
```{r}
# LAHKO NAREDIVA NOVO R DAToTEKO SAMO ZA KODO.

# TA KODA JE MARJANOVA IZ NAŠE PRVE NALOGE, KI SMO JO DOBILI NA VAJAH. PREVERJA, KAKO KORELACIJA MED POJASNJEVALNIMI SPREMENLJIVKAMI VPLIVA NA ŠIRINO IN POKRITOST IZ.
# KOPIRALA SEM JO SAMO ZA IZHODIŠČE.

n <- 100 # velikost vzorca
m <- 100 # stevilo vzorcev v simulacijah
r <- seq(from = 0, to = 0.9, by = 0.1) # korelacije, ki jih bomo upostevali

# pripravimo prostor za shranjevanje rezultatov:
R <- matrix(NA, nrow = length(r), ncol = 6)
rownames(R) <- r
b <- c(0.5, 1.2)

for (i in 1:length(r)){
  # pripravimo prostor za shranjevanje vmesnih rezultatov
  K <- matrix(NA, nrow = m, ncol = 6)
  for (j in 1:m){
    # generiramo podatke
    X <- mvrnorm(n = n, mu = c(2, 2), Sigma = rbind(c(1, r[i]), c(r[i], 1)))
    Y <- (X %*% b) + rnorm(n = n, mean = 0, sd = 1)
...
```{r eval=FALSE}
par(mfrow = c(1, 2))
matplot(R[,1:3], type = "l", xlab = "r", ylab = "pokritost IZ (delez vrednosti znotraj IZ)", xaxt="n")
axis(1, at = 1:10, labels = r)
legend("topleft", col = 1:3, legend = c("presecisce", "X1", "X2"), lty = 1:3)

matplot(R[,4:6], type = "l", xlab = "r", ylab = "sirina intervala zaupanja", xaxt="n")
axis(1, at = 1:10, labels = r)
legend("topleft", col = 1:3, legend = c("presecisce", "X1", "X2"), lty = 1:3)
```

```{r eval=FALSE}
# PRIMER GENERIRANJA POJASNJEVALNIH SPREMENLJIVK:
n=1000 # vzamemo dovolj velik vzorec, da vidimo, če smo res pravilno generirali podatke (da ne gre za slučaj)
...
- \emph{lme}, v: RDocumentation, [ogled 30.12.2020], dostopno na \url{https://www.rdocumentation.org/packages/nlme/versions/3.1-137/topics/lme}.
