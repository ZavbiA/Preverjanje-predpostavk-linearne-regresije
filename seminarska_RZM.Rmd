...
```{r fig.cap="Pokritost intervalov zaupanja za B1 pri polnem modelu GLM"}
#data_pokritost_polni_X25 <- data_pokritost_polni[data_pokritost_polni$alpha_X == 2,]
ggplot(data_pokritost_2[data_pokritost_2$model == 'glm'| data_pokritost_2$model == 'ols',], aes(x = alpha, y = value, col = model)) +
  facet_grid(n~korelacija+alpha_X) +
  stat_summary(fun = mean, geom="line") +
  stat_summary(fun = mean, geom="point") +
  labs(x = "stopnja asimetrije (alpha)", y = "pokritost IZ")
```
```{r fig.cap="Pokritost intervalov zaupanja za B1 pri polnem modelu"}
#data_pokritost_polni_X25 <- data_pokritost_polni[data_pokritost_polni$alpha_X == 2,]
ggplot(data_pokritost_2[data_pokritost_2$model == 'glm'| data_pokritost_2$model == 'ols',], aes(x = alpha, y = value, col = model)) +
  facet_grid(n~korelacija+alpha_X) +
  stat_summary(fun = mean, geom="line") +
...
```{r fig.cap="Širina intervalov zaupanja"}
sirina <- c("sirina_B0", "sirina_B1", "sirina_B2", "sirina_B3")
MeltedData <- melt(simulacija[simulacija$model == 'glm'| simulacija$model == 'ols',], id.vars = c("model","korelacija", "alpha", "n"), measure.vars = sirina)
...
```{r fig.cap="Pristranskost ocen"}
pristranskost <- c("B0", "B1", "B2", "B3")
MeltedData <- melt(simulacija[simulacija$model == 'glm'| simulacija$model == 'ols',], id.vars = c("model", "korelacija", "alpha", "n"), measure.vars = pristranskost)
ggplot(MeltedData, aes(x = alpha, y = value, col = variable)) +
  facet_grid(n~korelacija+model) +
  stat_summary(fun = mean, geom="line") +
  stat_summary(fun = mean, geom="point") +
