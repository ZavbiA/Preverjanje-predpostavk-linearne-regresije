...
Številke v tabelah potrjujejo naša opažanja. Med metodama `glm` in `ols` je pri pokritosti manjša razlika in sicer je pri dovolj velikih vzorcih tokrat pokritost pri metodi `glm` višja za do 1 o.t. Prav tako je manjša razlika v širini intervalov zaupanja. Ponovno pa velja, da se (procentualna) razlika v širini z večanjem velikosti vzorca povečuje vprid metode \emph{glm}.

## Vpliv odstranjevanja spremenljivk

Poglejmo si še vpliv izločanja spremenljivk. Zanima nas, kakšen je ta vpliv, ali se razlikuje med metodama in ali se razlikuje med različno asimetričnimi porazdelitvami pojasnjevalnih spremenljivk. 
...
Poglejmo si še razlike med metodama, kjer v modelu izključimo eno od spremenljivk. 
...
Rezultati so ponovno zelo podobni kot v primeru polnega modela.
...
Na Sliki 21 vidimo, da je pokritost IZ pri navadni linearni regresiji boljša kot pri linearnih modelih. Razlika je očitna predvsem pri manjših vzorcih, pri dovolj velikih pa praktično ni razlik (številke so razvidne v tabeli 17).
