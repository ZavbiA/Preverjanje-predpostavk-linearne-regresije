...
source("rezultati_simulacije_brez_norm.R")
...
```{r fig.cap="Pokritost intervalov zaupanja pri polnem modelu in Gamma(2,5) porazdelitvi pojasnjevalnih spremenljivk"}
...
```{r fig.cap="Pokritost intervalov zaupanja pri polnem modelu in Gamma(5,5) porazdelitvi pojasnjevalnih spremenljivk"}
...
Na zgornjem grafu opazujemo pokritost intervalov zaupanja regresijskega koeficienta $\beta_1$ pri polnem modelu (\emph{glm}), pri modelu, v katerega ne vključimo spremenljivke $X_2$ (*glm_x2*) ter pri modelu, v katerega ne vključimo spremenljivke $X_3$ (*glm_x3*).
...
```{r fig.cap="Širina intervalov zaupanja pri polnih modelih in Gamma(2,5) porazdelitvi pojasnjevalnih spremenljivk"}
...
```{r fig.cap="Širina intervalov zaupanja pri polnih modelih in Gamma(5,5) porazdelitvi pojasnjevalnih spremenljivk"}
...
ggplot(data_sirina_2[data_sirina_2$model == 'glm'| data_sirina_2$model == 'glm_x2'| data_sirina_2$model == 'glm_x3',], aes(x = alpha, y = value, col = model)) +
  ...
  stat_summary(fun = mean, geom="line") +
  stat_summary(fun = mean, geom="point") +
  labs(x = "stopnja asimetrije ostankov (alpha)", y = "širina IZ")
```

Na Sliki 5 so prikazane širine IZ regresijskega koeficienta $\beta_1$ z uporabo funkcije \emph{glm} pri polnem modelu, pri modelu brez $X_2$ (*glm_x2*) in pri modelu brez $X_3$ (*glm_x3*).
...
model1 <- aov(pokritost_B1 ~ model * n * korelacija * alpha, data = as.data.frame(simulacija))
